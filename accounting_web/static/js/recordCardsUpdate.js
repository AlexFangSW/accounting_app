...
async function updateRecord(recordId, crsfToken, data) {
    ...
    body: {
        ...
        "tag_name": `http://127.0.0.1:8000/api/tags/${data.tag-id}/`,
...
function getData(dom) {
    return {
        ...
        'tag' : dom.querySelector('div[class*=input-group][class*=tags]:not(.d-none) select option:checked').value,
        'tag-id' : dom.querySelector('div[class*=input-group][class*=tags]:not(.d-none) select option:checked').getAttribute('tag-id'),
...
let recordUpdateModalList = document.querySelectorAll('button.record-card-update');
